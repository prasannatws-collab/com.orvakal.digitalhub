# Orvakal Digital Hub – Production Deployment Script
...
# Step 4: Configure EAS Updates
Write-Host "`n=== Configuring EAS Updates ===" -ForegroundColor Cyan
eas update:configure --non-interactive 2>&1

# Step 5: Git commit configuration changes
Write-Host "`n=== Git Commit ===" -ForegroundColor Cyan
git add app.json eas.json
git status
git commit -m "chore: configure EAS updates and production build profiles"
Write-Host "Please run 'git push origin main' to push configuration changes." -ForegroundColor Yellow

Write-Host "`n=== All configuration and setup completed! ===" -ForegroundColor Green
Write-Host "Next steps (run manually in your terminal):" -ForegroundColor Yellow
Write-Host "1. Deploy to Expo (EAS Update):" -ForegroundColor Cyan
Write-Host "   eas update --branch production --message 'Initial production release'" -ForegroundColor White
Write-Host "2. Build Production Android App Bundle:" -ForegroundColor Cyan
Write-Host "   eas build --platform android --profile production" -ForegroundColor White
Write-Host "3. Submit Build to Google Play Store:" -ForegroundColor Cyan
Write-Host "   eas submit --platform android --profile production --latest" -ForegroundColor White
